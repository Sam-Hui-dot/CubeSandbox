...
4. **Web 服务路由**：demo 通过 `sandbox.get_host(8080)` 走 CubeSandbox 路由访问
   Spring Boot 服务。
5. **受限出口场景准备**：动态 Maven 下载用于开发模式；生产或受限出口环境可以预构建依赖、
   使用内部 Maven 镜像，或从受控网络阶段创建预热缓存快照。
...
- `Dockerfile`：基于 `cubesandbox-base` 的 Java 21 + Maven 镜像，并预热 Maven 依赖缓存。
...
2. 上传 Spring Boot 项目到 `/workspace/java-springboot-web`。
3. 使用模板预热的 Maven 缓存执行 `mvn --offline -DskipTests package`，构建 jar。
4. 启动服务，并通过 CubeSandbox 路由调用 `/health`、`/api/info` 和
   `POST /api/tasks`。
5. 验证任务状态已写入 `/tmp/cubesandbox-spring/state/tasks.json`。
6. 停止服务并创建 checkpoint snapshot。
7. 从 snapshot 启动一个新沙箱。
8. 验证 `/workspace/.m2/repository`、`target/*.jar` 和任务状态文件都被继承。
9. 直接用继承的 jar 启动 Spring Boot，并读取原始任务。
10. 下载 `output/manifest.json`，作为本次运行的结构化证明。
...
Dockerfile 在模板构建阶段完成 Maven 依赖预热，运行时 demo 使用 Maven offline 模式。生产、
合规或受限出口集群建议使用以下方式之一：
...
- 首次 Maven 构建需要网络，除非依赖已预构建进模板或来自内部镜像。
