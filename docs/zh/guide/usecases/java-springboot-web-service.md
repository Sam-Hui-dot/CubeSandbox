...
模板使用 digest 固定 CubeSandbox 和 Maven 基础镜像，并安装 Java 21、Maven、curl 和 bash。
示例 Spring Boot 项目暴露四个端点：
...
1. 使用 `allow_internet_access=False` 创建沙箱。
2. 证明公网 HTTPS 请求被阻断。
3. 上传 Spring Boot 项目。
4. 使用模板预热的 `/workspace/.m2/repository` 执行
   `mvn --offline -DskipTests package`，构建 `target/*.jar`。
5. 启动服务，通过 CubeSandbox 的 `8080` 端口路由访问它。
6. 创建任务，并验证状态文件存在。
7. 停止 JVM、确认进程退出、落盘状态，再创建 checkpoint snapshot。
8. 从 checkpoint fork 一个新的默认拒绝出口沙箱。
9. 验证 fork 后继承了 Maven cache、构建好的 jar 和任务状态。
10. 直接从继承的 jar 启动 Spring Boot，并读取原始任务。
11. 下载 manifest，证明出口、cache、artifact、state 和路由访问都已验证。
...
- 证明公网出口被拒绝时 Maven 离线构建仍可成功。
- 产出一个小型 JSON manifest，方便 reviewer 检查运行结果。
- v1 不引入外部数据库和多容器编排，保持范围聚焦。

## 受限出口场景

Dockerfile 在受控的模板构建阶段完成 Maven 依赖预热。运行时 demo 先在
`allow_internet_access=False` 下证明公网 HTTPS 被阻断，再完成 Maven offline 构建。生产或
合规集群可以用以下三种方式适配：
